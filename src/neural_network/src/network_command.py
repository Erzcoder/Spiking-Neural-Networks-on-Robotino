...
def test_callback(data_input):
    ...
    pub = rospy.Publisher('network_command', numpy_msg(Floats), queue_size=10)
    # construct the output command
        
    if data_input.angular.z==0 :
        msg=np.array([0,0], dtype=np.float32)
        
    elif data_input.angular.z>0 :
        msg=np.array([data_input.angular.z,0], dtype=np.float32)
        
    elif data_input.angular.z<0 :
        msg=np.array([0,-data_input.angular.z], dtype=np.float32)
