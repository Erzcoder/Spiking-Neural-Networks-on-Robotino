...
def test_callback(data_input):
    ...
    input=p.Population(1,p.SpikeSourcePoisson, {'rate':msg_list[1]/3})
    ...
    command.linear.x = np.abs((int(mean_rate)%10-4))*1+0.4 #0.24
    #command.angular.z = -(int(mean_rate)%10-4.5)*10
    command.angular.z = -(int(mean_rate)%10-4.5)
